...
client.on('group-participants-update', async (anu) => {
	...
	ppimg = 'https://i0.wp.com/www.gam,nik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
	}
...
const isAntiVO = isGroup ? antiviewonce.includes(from) : false	
		...
		const katalog = (teks) => {
             res = pemuda.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "© Kiriyu Botz", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pemuda.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pemuda.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "© Kiriyu Botz", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "© Kiriyu Botz", [{buttonId: 'simi', buttonText: {disText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
	...
	//•======>> SUBSCRIBE : PEMUDA TEKNOLOGI <<====•\\
        ...
        const sendWebp = async(from, url) => {
	        ...
	        pemuda.sendMessage(from, { disname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
				...
				const reply = (teks) => {
			pemuda.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Hai ${pushname}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6281337744739?text=.menu`}}})
		}
		const simir = (teks) => {
...
if (!isQuotedSticker) return reply(`Reply Stikernya`)
	...
	pempa = anee[0] !== "" ? anee[0] : `It's Me `;
	autpa = typeof anee[1] !== "undefined" ? anee[1] : `YT : Kiriyu Botz`;
...
case 'ppcp': 
   ...
   sendButImage(from, pll, `Kiriyu Botz © 2021`, cewe, [                      
          {
...
break
		case 'instainfo':
		pemuda.sendMessage(from, { text: "Belum ada kak\n\...._", matchedText: '...._', description: "", title: "Kembali Ketik .menu", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: true, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
case 'youtubeinfo':
		pemuda.sendMessage(from, { text: "Belum ada kak_", matchedText: '...._', description: "", title: "Kembali Ketik .menu", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: true, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
		
case 'partnerbot':
		pemuda.sendMessage(from, { text: "*Partner BOT Kiriyu*\n\n- Bot Kiriyu :\nhttps://wa.me/6281337744739?text=.menu\nThanks Udah Makek Bot ini😉", matchedText: 'https://youtu.be/x-O0WHkv3uc', description: "", title: "Yang klik Anak Haram", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
case 'speed':
               case 'ping':
...
Giliran = @${tty.er1.split('@')[0]}`
 ...
 //•======>> SUBSCRIBE : PEMUDA TEKNOLOGI <<====•\\
     ...
     pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Kiriyu",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://youtu.be/x-O0WHkv3uc"}}})
	}
    }
    }
    }
